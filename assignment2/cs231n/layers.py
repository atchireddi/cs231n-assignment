...
def conv_forward_naive(x, w, b, conv_param):
  ...
  - w: Filter weights of shape (F, C, HH, WW)   # F is the depth of the out layer, HH x WW is the size of receptive field
  - b: Biases, of shape (F,)
...
# Brut force love the loops !
  ...
  for fprime in range(F):
      for cprime in range(C):
          for i in range(HH):
              for j in range(WW):
                  sub_xpad = x_pad[:, cprime, i:i + Hh * S:S, j:j + Hw * S:S] # these elements will multiply w[fprime,cprime,i,j]
